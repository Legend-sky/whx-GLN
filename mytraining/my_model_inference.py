...
class RetroGLN(object):
    ...
    def my_run(self, raw_prod,list_of_reacts):
        cano_prod = DataInfo.get_cano_smiles(raw_prod)
        prod_mol = SmilesMols.get_mol_graph(cano_prod)
        list_rxns = []
        for i in range(len(list_of_reacts)):
            list_rxns.append([DataInfo.get_cano_smiles(r) + '>>' + cano_prod for r in list_of_reacts[i]])
        
        react_scores = self.gln.reaction_predicate.inference([prod_mol] * len(list_rxns), list_rxns)
        react_scores = react_scores.view(-1).data.cpu().numpy()

        idx = 0
        final_joint = []
        for reacts in list_of_reacts:
            r_score = react_scores[idx]
            final_joint.append((r_score,reacts))
            idx += 1
        # final_joint = sorted(final_joint, key=lambda x: -1 * x[0])
        scores = [t[0] for t in final_joint]
        scores = softmax([each_score for each_score in scores])
        list_reacts = [t[1] for t in final_joint]
        return scores, list_reacts